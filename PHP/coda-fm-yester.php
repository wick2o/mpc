...
for ( $counter = 1; $counter <= $max; $counter += 1) {
	...
	{
		if ( strpos($artist_return_array['FILE'], "<h4>Added yesterday</h4>") == true )
		{
			$artist_data = parse_array($artist_return_array['FILE'], "<h4>Added yesterday</h4>" ,"<div class='link' id='paginator'>");
		}
		else
		{
			$artist_data = parse_array($artist_return_array['FILE'], "<h4>Added yesterday</h4>" ,"<h4>Added a few days ago</h4>");
			$last_page = "true";
		}
	}
